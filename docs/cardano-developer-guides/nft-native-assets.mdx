...
1. **Download the Cardano node genesis file from the Cardano Book**
...
```bash title=">_ Terminal"
wget -N https://book.world.dev.cardano.org/environments/preprod/shelley-genesis.json -O testnet-shelley-genesis.json
```
...
```bash title=">_ Terminal"
wget -N https://book.world.dev.cardano.org/environments/mainnet/shelley-genesis.json -O mainnet-shelley-genesis.json
```
...
If you are using testnet make sure you have the correct testnet-magic version number. You can find the current testnet version [here](https://book.world.dev.cardano.org/environments/preprod/shelley-genesis.json) or simply look in your <span style={{color: 'blue'}}>testnet-shelley-genesis.json</span> file in your cardano node directory.
